              ...
              <v-layout align-center fill-height>
                <v-flex shrink mr-2 v-if="tab === 'estimate'">
                  <input
                    ...
                    @keydown.up.exact.prevent="$event => submitEstimate($event, item.id, 'previousItem')"
                    @keyup.down.exact.prevent="$event => submitEstimate($event, item.id, 'nextItem')"
                    @keydown.tab.exact.prevent="$event => submitEstimate($event, item.id, 'nextItem')"
                    @keyup.enter.exact.prevent="$event => submitEstimate($event, item.id, 'nextItem')"
